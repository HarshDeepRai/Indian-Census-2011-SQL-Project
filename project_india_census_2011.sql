...
/* population in india*/
 select sum(population) as p from project.dataset2;
 /*avg growth*/
 select avg(growth) from project.dataset1;
/*avg growth state wise*/
 select state,avg(growth) from project.dataset1 group by state order by avg(growth);
/*avg sex ratio*/
...
select state,round(avg(literacy),0) as literacy from project.dataset1 group by state having literacy>90 order by literacy desc;
/*select top 3 states having highest growth rate*/
select state,avg(growth) from project.dataset1 group by state order by avg(growth) desc limit 3;
/*select bottom 3 states having lowest growth rate*/
select state,avg(growth) from project.dataset1 group by state order by avg(growth) limit 3;
/*select bottom 3 states having lowest sex ratio*/
select state,round(avg(sex_ratio)) as avg_sex_ratio from project.dataset1 group by state order by avg_sex_ratio limit 3;
...
select state,round(avg(literacy),0) as literacy from project.dataset1 group by state order by literacy desc;
...
select state,round(avg(literacy),0) as literacy from project.dataset1 group by state order by literacy;
...
select DISTINCT STATE from project.dataset1 where LEFT(State,1)='a' or LEFT(STATE,1)='b';
/*states starting with A and ending with h*/
select distinct state from project.dataset1 where lower(state) like 'a%h';


/*number of male and females in every city*/
select c.district,c.state,round(c.population/(c.sex_ratio+1),0) males,round((c.population*c.sex_ratio)/(c.sex_ratio+1),0) females from
(select a.district,a.state,a.sex_ratio/1000 as sex_ratio,b.population from project.dataset1 a inner join project.dataset2 b on a.district=b.district)c;
/*total literacy rate*/
...
(select a.district,a.state,a.literacy/1000 as literacy,b.population from project.dataset1 a inner join project.dataset2 b on a.district=b.district)c order by literate desc)d group by state order by state_literate;
...
(select a.district,a.state,a.growth Growth,b.population from project.dataset1 a inner join project.dataset2 b on a.district=b.district)d)e group by e.state)f;
...
(select e.state,sum(e.previous_census) as previous_census,sum(e.present_population) as present_census from
(select d.district,d.state,round(d.population/(1+growth),0) as previous_census,d.population as present_population from
(select a.district,a.state,a.growth Growth,b.population from project.dataset1 a inner join project.dataset2 b on a.district=b.district)d)e group by e.state)f)g)q
inner join
(select '1' as keyy,z.* from 
(select sum(area_km2) total_area from project.dataset2)z)r on q.keyy=r.keyy)c;
